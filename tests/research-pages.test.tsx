...
describe("research institute pages", () => {
  it("lists crawlable research articles by topic without presenting as company news", () => {
    ...
    expect(screen.getByRole("heading", { level: 2, name: "专题分类" })).toBeInTheDocument();
    ...
    expect(screen.getAllByRole("heading", { level: 2, name: "为什么品牌需要重新理解私域用户运营" }).length).toBeGreaterThan(0);
    expect(screen.getAllByText(/私域不是把用户拉进群/).length).toBeGreaterThan(0);
    expect(screen.getAllByRole("link", { name: "阅读为什么品牌需要重新理解私域用户运营" })[0]).toHaveAttribute(
      "href",
  ...
  it("renders a research article detail page as crawlable expert content", async () => {
    ...
    expect(screen.getByRole("heading", { level: 2, name: "文章导读" })).toBeInTheDocument();
